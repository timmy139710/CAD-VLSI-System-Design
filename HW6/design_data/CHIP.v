...
module CHIP ( clk, reset, Din, in_en, busy, out_valid, Dout );
input clk, reset, in_en;
input [7:0] Din;
output busy, out_valid;
output [7:0] Dout;

wire i_clk, i_reset, i_in_en;
wire [7:0] i_Din;
wire i_busy, i_out_valid;
wire [7:0] i_Dout;

LMFE LMFE(.clk(i_clk), .reset(i_reset), .in_en(i_in_en), .Din(i_Din), .busy(i_busy), .out_valid(i_out_valid), .Dout(i_Dout));

PDIDGZ ipad_clk (.PAD(clk), .C(i_clk));
PDIDGZ ipad_reset (.PAD(reset), .C(i_reset));
PDIDGZ ipad_in_en (.PAD(in_en), .C(i_in_en));
PDIDGZ ipad_Din0 (.PAD(Din[0]), .C(i_Din[0]));
PDIDGZ ipad_Din1 (.PAD(Din[1]), .C(i_Din[1]));
PDIDGZ ipad_Din2 (.PAD(Din[2]), .C(i_Din[2]));
PDIDGZ ipad_Din3 (.PAD(Din[3]), .C(i_Din[3]));
PDIDGZ ipad_Din4 (.PAD(Din[4]), .C(i_Din[4]));
PDIDGZ ipad_Din5 (.PAD(Din[5]), .C(i_Din[5]));
PDIDGZ ipad_Din6 (.PAD(Din[6]), .C(i_Din[6]));
PDIDGZ ipad_Din7 (.PAD(Din[7]), .C(i_Din[7]));

PDO12CDG opad_busy (.I(i_busy), .PAD(busy));
PDO12CDG opad_out_valid (.I(i_out_valid), .PAD(out_valid));
PDO12CDG opad_Dout0 (.I(i_Dout[0]), .PAD(Dout[0]));
PDO12CDG opad_Dout1 (.I(i_Dout[1]), .PAD(Dout[1]));
PDO12CDG opad_Dout2 (.I(i_Dout[2]), .PAD(Dout[2]));
PDO12CDG opad_Dout3 (.I(i_Dout[3]), .PAD(Dout[3]));
PDO12CDG opad_Dout4 (.I(i_Dout[4]), .PAD(Dout[4]));
PDO12CDG opad_Dout5 (.I(i_Dout[5]), .PAD(Dout[5]));
PDO12CDG opad_Dout6 (.I(i_Dout[6]), .PAD(Dout[6]));
PDO12CDG opad_Dout7 (.I(i_Dout[7]), .PAD(Dout[7]));
